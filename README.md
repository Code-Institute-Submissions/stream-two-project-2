...
Additionally, table cells containing flag icons are displaying out of line by one pixel in Chrome when the display is more than 800 pixels wide. This too is not replicated in Firefox and the developer tools offer no clue as to why it might be happening, so it may be another bug in the Chrome browser.